...
- **症状**: 修仙运行数小时但任务进度为0%
- **原因**: 工具链中断或循环陷入无操作状态
- **解决**: 使用 `/修仙 status` 查看当前状态，`/修仙 log` 查看最后操作；如果步骤停滞，调用 `/自控 resume` 恢复上下文

### 终端崩溃
- **症状**: opencode TUI 崩溃，显示 `setRawMode errno: 9`
- **原因**: bash超时触发SIGKILL导致stdin FD损坏
- **解决**: 重启 opencode GUI，检查 `~/.local/share/opencode/log/` 最后日志，增加timeout设置

### 预算耗尽
- **症状**: 任务未完成但资源耗尽
- **原因**: 预算设置不足或任务范围过大
- **解决**: 使用 `--budget high` 重启，或使用 `/修仙 status` 查看已完成进度恢复执行

### 模型切换失败
- **症状**: 自动模型切换后输出质量下降
- **原因**: 备用模型能力不足
- **解决**: 检查模型配置，确保至少有2个可靠模型可用

## 边界情况

- **空仓库启动**: 先初始化项目结构，再执行任务
- **只读文件系统**: 检测写权限失败时，跳过文件操作，仅输出分析结果
- **网络不可用**: 降级为离线模式，仅使用本地工具和缓存数据
- **大文件处理**: 超过10MB的文件自动转为分析模式，不做全文处理
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.0.0 | 2026-04-01 | 初始版本，8种任务模式，自主循环 |
| 1.1.0 | 2026-05-09 | 添加安全规则，不可逆操作确认机制，终端崩溃防护 |
