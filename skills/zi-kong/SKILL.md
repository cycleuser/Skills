...
### 内存优化
- **上下文压缩**: 超过50轮对话时，压缩前20轮为摘要，仅保留核心决策点
- **规则按需加载**: 不预加载所有规则文件，仅在需要时读取对应规则
- **缓存复用**: 同一会话中的重复API调用结果缓存10分钟

### 预算智能分配
- **前期密集**: 前30%预算用于深入分析问题、设计方案
- **中期执行**: 40%预算用于实施和迭代修改
- **后期验证**: 30%预算用于测试验证和文档更新
- **动态调整**: 如果某个阶段超时，自动从后续阶段借调预算

### 并行任务调度
- **独立任务并行**: 不互相依赖的任务同时执行
- **依赖任务串行**: 有依赖关系的任务按拓扑排序串行
- **资源限制**: 同时最多3个并行子任务，避免上下文爆炸

### 检查点与回滚
- **自动检查点**: 每次文件修改前自动创建检查点
- **增量存储**: 检查点仅存储变更差异，不复制整个项目
- **回滚成本**: 检查点保留最近5个，更早的自动清理

### 跨会话持久化
- **精简状态**: 仅保存任务描述、当前阶段、关键决策树，不保存完整上下文
- **恢复速度**: 跨会话恢复时，先加载状态摘要，再按需重建完整上下文
- **会话合并**: 如果检测到多个会话处理同一任务，自动合并进度
